...
from server.gizmoapp_server.api import _chat_messages
...
class GizmoAppTestCase(unittest.TestCase):
    ...
    def test_chat_history_forwards_recent_assistant_replies_with_total_bound(self):
        history = [
            {"question": f"old question {index}", "answer": "old answer " + ("x" * 12_000)}
            for index in range(8)
        ]

        messages = _chat_messages(history, "current question")
        contents = [message["content"] for message in messages]

        self.assertEqual(messages[0]["role"], "system")
        self.assertEqual(messages[-1], {"role": "user", "content": "current question"})
        self.assertIn("old question 7", contents)
        self.assertTrue(any("old answer" in content for content in contents))
